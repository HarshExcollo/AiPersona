...
export default function ChatPage({ onBack }: ChatPageProps) {
  ...
  const handleSendMessage = async (e?: React.FormEvent) => {
    ...
    if (isWebhookPersona(persona.id)) {
      try {
        // Add a typing indicator or loading state
        setMessages((prev) => [...prev, { sender: 'ai', text: "Processing your query..." }]);
        
        // Call the webhook
        const webhookResponse = await sendToWebhook(trimmed, persona.id, persona.name);
        
        // Replace the loading message with the actual response
        setMessages((prev) => {
          const newMessages = [...prev];
          newMessages[newMessages.length - 1] = { sender: 'ai', text: webhookResponse };
          return newMessages;
        });
      ...
      // For other personas, use the default response
      setTimeout(() => {
  ...
  const suggestionChips = React.useMemo(() => getSuggestionChips(persona.department, persona.id), [persona]);
        ...
        >
          ...
          <Box sx={{
            ...
            <Box sx={{ 
              ...
              width: '100%',
              px: { xs: 2, sm: 3 },
              mt: { xs: 2, sm: 3 }, // Increased top margin for better spacing
            }}>
